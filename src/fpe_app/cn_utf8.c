...
int aux_fpe_cn(WBCRYPTO_fpe_app_context *ctx, char *cn, char *sample, char *after_cn, fpe_block128_f block) {
    ...
    if (strcmp(ctx->cipher, WBCYRPTO_FPE_CIPHER_SM4) == 0) {
        ...
        fpe_ctx = WBCRYPTO_sm4_lut_fpe_init(ctx->cipher_ctx, tweak, sizeof(tweak), 12);
    } else if (strcmp(ctx->cipher, WBCYRPTO_FPE_CIPHER_WBSM4_SE_LA) == 0) {
        fpe_ctx = wbcrypto_wbsm4_se_la_fpe_init(ctx->cipher_ctx, tweak, sizeof(tweak), 12);
        ...
        fpe_ctx = WBCRYPTO_sdf_sm4_fpe_init(ctx->cipher_ctx, tweak, sizeof(tweak), 12);
    }else { // default: aes
        fpe_ctx = WBCRYPTO_aes_fpe_init(ctx->cipher_ctx, tweak, sizeof(tweak), 12);
    }
