 ...
 * @Date: 2023-05-21 10:03:47
 * @LastEditors: error: error: git config user.name & please set dead value or install git && error: git config user.email & please set dead value or install git & please set dead value or install git
 * @LastEditTime: 2023-05-21 10:05:28
 * @FilePath: /wbcrypto-fpe/src/fpe_app/idcard.c
...
int aux_fpe_idcard(WBCRYPTO_fpe_app_context *ctx, char *idcard, char *sample, char *after_idcard, fpe_block128_f block) {
    ...
    if (strcmp(ctx->cipher, WBCYRPTO_FPE_CIPHER_SM4) == 0) {
        ...
        fpe_ctx = WBCRYPTO_sm4_lut_fpe_init(ctx->cipher_ctx, tweak, sizeof(tweak), 10);
    } else if (strcmp(ctx->cipher, WBCYRPTO_FPE_CIPHER_WBSM4_SE_LA) == 0) {
        fpe_ctx = wbcrypto_wbsm4_se_la_fpe_init(ctx->cipher_ctx, tweak, sizeof(tweak), 10);
        ...
        fpe_ctx=WBCRYPTO_wbsm4_xl_la_fpe_init(ctx->cipher_ctx,tweak,sizeof(tweak),10);
        ...
        fpe_ctx = WBCRYPTO_sdf_sm4_fpe_init(ctx->cipher_ctx, tweak, sizeof(tweak), 10);
    }else { // default: aes
        fpe_ctx = WBCRYPTO_aes_fpe_init(ctx->cipher_ctx, tweak, sizeof(tweak), 10);
    }
