...
{
    struct wbcrypto_sm4_lut_context *ctx = malloc(sizeof(struct wbcrypto_sm4_lut_context));
    if (ctx == NULL) {
        WBCRYPTO_THROW_REASON("WBCRYPTO_sm4_lut_context_init", WBCRYPTO_ERR_ALLOC_FAILED);
        goto cleanup;
    }
    memset(ctx, 0, sizeof(struct wbcrypto_sm4_lut_context));
    return ctx;
cleanup:
    return NULL;
}

WBCRYPTO_fpe_context *WBCRYPTO_sm4_lut_fpe_init(wbcrypto_sm4_lut_context *sm4_ctx, const char *twkbuf, size_t twklen, unsigned int radix)
{
    WBCRYPTO_fpe_context *ctx = WBCRYPTO_fpe_init(twkbuf, twklen, radix, sm4_ctx, (block128_f)wbcrypto_sm4_lut_encrypt);
    return ctx;
}
