...
int WBCRYPTO_wbsm4_xl_la_file2key(WBCRYPTO_wbsm4_xl_la_context *ctx, char *fpath) {
    ...
    if ((rkey = fopen(fpath, "rb")) == NULL) {
        WBCRYPTO_THROW_REASON("WBCRYPTO_wbsm4_xl_la_key2file", WBCRYPTO_ERR_FILE_IO);
    }
    fread(ctx, sizeof(WBCRYPTO_wbsm4_xl_la_context), 1, rkey);
    ret = 1;
cleanup:
    fclose(rkey);
    return ret;
...
int WBCRYPTO_wbsm4_xl_la_key2file(const WBCRYPTO_wbsm4_xl_la_context *ctx, char *fpath) {
    ...
    if ((wkey = fopen(fpath, "wb+")) == NULL) {
        WBCRYPTO_THROW_REASON("WBCRYPTO_wbsm4_xl_la_file2key", WBCRYPTO_ERR_FILE_IO);
    }
    fwrite(ctx, sizeof(WBCRYPTO_wbsm4_xl_la_context), 1, wkey);
    ret = 1;
cleanup:
    fclose(wkey);
    return ret;
...
//        WBCRYPTO_THROW_REASON("WBCRYPTO_wbsm4_xl_la_key2file", WBCRYPTO_ERR_READ_BAD_FILE);
//    }
//    for (i = 0; i < 32; i++) {
//        for (j = 0; j < 3; j++) {
//            for (k = 0; k < 4; k++) {
...
//        WBCRYPTO_THROW_REASON("WBCRYPTO_wbsm4_xl_la_file2key", WBCRYPTO_ERR_READ_BAD_FILE);
//        goto cleanup;
//    }
